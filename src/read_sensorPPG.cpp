...
static const unsigned long SAMPLE_PERIOD_MS = 200; // Định kỳ đúng 20ms (50 Hz) chuẩn xác
...
static uint8_t current_red_amp = 30; //
static uint8_t current_ir_amp = 30;  //
static uint32_t full_scale_adc = 131071;

// buffer luu tru rawIR[idxRaw] va rawRed[idxRaw]
uint32_t rawRed[256] = {0};
uint32_t rawIR[256] = {0};
uint32_t idxRaw = 0;
void agc_amplitude_led(uint32_t ppgRed, uint32_t ppgIR)
{
    int agc_cooldown_counter = 0;
    while (1)
    {

        if (agc_cooldown_counter < SAMPLE_PERIOD_MS) //
        {
            ...
            Serial.print(", "); //
            Serial.print(current_ir_amp);
            Serial.println("."); //
            // Chỉnh dựa trên dc_track thay vì raw để không bị méo biên độ bởi sóng AC
            if (ppgRed > 0.6 * full_scale_adc && current_red_amp > 10) //
            {
                if (ppgRed > 240000.0f)
                    current_red_amp -= 5;
                else
            ...
            else if (ppgRed < 0.4 * full_scale_adc && current_red_amp < 245) //
            {
                current_red_amp++;
                is_hardware_adjusted = true; //
            }

            if (ppgIR > 0.6 * full_scale_adc && current_ir_amp > 10) //
            {
                if (ppgIR > 240000.0f)
                    current_ir_amp -= 5;
                else
            ...
            else if (ppgIR < 0.4 * full_scale_adc && current_ir_amp < 245) //
            {
                current_ir_amp++;
                is_hardware_adjusted = true; //
            }
...
void ppgSensor_process(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        {
            rawIR[idxRaw] = ppgSensor.getFIFOIR();
            rawRed[idxRaw] = ppgSensor.getFIFORed();
            idxRaw++;
            ppgSensor.nextSample();
        }
        // BẪY LỖI PHẦN CỨNG: PHÁT HIỆN HỞ SÁNG HOẶC NHẤC NGÓN TAY RA KHỎI CẢM BIẾN
        if (rawIR[idxRaw] < 40000 || rawRed[idxRaw] < 40000) //
        {
            sampleCount = 0;   //
            strideCounter = 0; //
            circular_index = 0;
            ...
            // if (rawIR[idxRaw] > 255000)
            // {
            //     current_ir_amp -= 5;
            //     ppgSensor.setPulseAmplitudeIR(current_ir_amp);
            // }
            // if (rawRed[idxRaw] > 255000)
            // {
            //     current_red_amp -= 5;
            //     ppgSensor.setPulseAmplitudeRed(current_red_amp);
            // }
        ...
        if (!isFingerAttached) //
        {
            dc_track_red = (float)rawRed[idxRaw]; //
            dc_track_ir = (float)rawIR[idxRaw];   //
            lpf_red_prev = 0.0f;                  //
            lpf_ir_prev = 0.0f;                   //
        ...
        // ----------------------------------------------------------------
        float acRed_raw = (float)rawRed[idxRaw] - dc_track_red;                               //
        dc_track_red = (1.0f - HPF_ALPHA) * (float)rawRed[idxRaw] + HPF_ALPHA * dc_track_red; //

        float acIR_raw = (float)rawIR[idxRaw] - dc_track_ir;                               //
        dc_track_ir = (1.0f - HPF_ALPHA) * (float)rawIR[idxRaw] + HPF_ALPHA * dc_track_ir; //
        ...
        if (isBufferFull && (strideCounter >= STRIDE_SIZE)) //
        {
            ...
            if (rmsIR > 0.0f && dc_track_red > 0.0f && dc_track_ir > 0.0f) //
            {
                Serial.print("HERE!!!");
                // Tỷ số R tức thời (Ratio-of-Ratios)
                float instant_R = (rmsRed / dc_track_red) / (rmsIR / dc_track_ir); //
                ...
                {
                    final_SpO2 = 0.4f * instant_SpO2 + 0.6f * final_SpO2; // Lọc thông thấp IIR
                }
            }
            ...
            Serial.print(now);
            Serial.print(","); //
            Serial.print(rawRed[idxRaw]);
            Serial.print(","); //
            Serial.print(rawIR[idxRaw]);
            Serial.print(","); //
...
void ppg_RawData(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        while (ppgSensor.available())
        {
            redRaw = ppgSensor.getFIFORed();
            irRaw = ppgSensor.getFIFOIR();

            ppgSensor.nextSample();
        }

        Serial.printf("ir: %lu red: %lu\n", (unsigned long)irRaw, (unsigned long)redRaw);
        vTaskDelay(pdMS_TO_TICKS(20));
    }
}
